-- Allow chat leads without a phone (name-only intake). Run after 21_chat_leads_unit_message.sql.
...
begin
  ...
  phone_trim := trim(coalesce(p_phone, ''));
  if phone_trim <> '' and length(phone_trim) < 7 then
    return json_build_object('ok', false, 'error', 'Phone number looks too short.', 'id', null);
  end if;
